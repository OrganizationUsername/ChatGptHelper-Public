...
public class ImageSearchThing
{
    ...
    public List<BitmapImage> Images { get; set; } = new();

    public static ImageSearchThing Get(ImageQuery iq)
    {
        var sth = new ImageSearchThing() { QueryText = iq.ImageQueryText };
        foreach (var imageData in iq.ImageResults)
        {
            ...
            image.EndInit();
            image.Freeze(); // Important for use in a multi-threaded environment
            sth.Images.Add(image);
        }
